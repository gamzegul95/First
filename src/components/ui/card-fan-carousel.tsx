...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  // Auto-advance through the deck like a slideshow when there are more cards
  // than fit on screen. Pauses on hover and when the tab isn't visible.
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (paused || document.hidden || isAnimating.current) return;
      cycle("right");
    }, 4500);
